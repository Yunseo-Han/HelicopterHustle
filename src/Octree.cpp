...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < childBoxes.size(); ++i) {
		Box box = childBoxes.at(i);
		vector<int> boxPoints;
		getMeshPointsInBox(mesh, node.points, box, boxPoints);

		if (boxPoints.size() > 0) {
			node.children.emplace_back(TreeNode());
			node.children.back().box = box;
			node.children.back().points = boxPoints;
			if (boxPoints.size() > 1) subdivide(mesh, node.children.back(), numLevels, level + 1);
		} 
	}
}
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	if (!node.box.intersect(ray, 0.1, 1000)) return false;
